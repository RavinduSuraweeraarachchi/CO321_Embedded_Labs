...
volatile uint8_t ovf_count0 = 0; // For TIMER0
volatile uint16_t ovf_count1 = 0; // For TIMER1
...
ISR(TIMER0_OVF_vect) 
{
    ...
    TCNT0 = 0; // Reload the timer counter register
    ovf_count0++;
    if (ovf_count0 >= 12) {
...
ISR(TIMER2_OVF_vect) 
{
    ...
    TCNT2 = 0; // Reload the timer counter register
    ovf_count2++;
    if (ovf_count2 >= 122) {
